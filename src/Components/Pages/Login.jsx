...
const Login = () => {
    ...
    return (
        <div className="container w-50 py-4 my-5 border rounded shadow p-3 bg-body-tertiary">
            {connected === true ? <Message style={{border : '0.1px solid green'}} type="success" closable >Connected Successfully.</Message> : connected === false ? <Message style={{border : '0.1px solid red'}} type="error" closable >Username of password is incorrect.</Message> : '' }
                ...
                <div className="rounded">
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="form-outline mb-2">
                            <label className={`form-label text-${connected === false ? 'danger' : '' }`} htmlFor="form2Example1">Email address or username</label>
                            <input type="text" id="form2Example1" className={`form-control border-${connected === false ? 'danger' : '' }`} onChange={(e) =>dispatch({type: 'EMAIL', payload : e.target.value})} />
                        </div>
                        
                        <div className="form-outline mb-2">
                            <label className={`form-label text-${connected === false ? 'danger' : '' }`} htmlFor="form2Example2">Password</label>
                            <input type="password" id="form2Example2" className={`form-control border-${connected === false ? 'danger' : '' }`}  onChange={(e) =>dispatch({type: 'PASSWORD', payload : e.target.value})} />
                        </div>
